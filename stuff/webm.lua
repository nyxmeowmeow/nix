	...
	local _base_0 = {
		draw = function(self)
			...
			if self.attempt > 0 then
				ass:append(
					"Encoding ("
						.. tostring(bold(progressText))
						.. ")\\NAttempt: "
						.. tostring(self.attempt)
						.. "\\NCRF: "
						.. tostring(self.crf)
						.. "\\NLast Size: "
						.. tostring(self.lastSize)
						.. "\\NTrgt Size: "
						.. tostring(self.target)
						.. "\\NRatio: "
						.. tostring(self.lastSize / self.target)
				)
...
encodeWithTarget = function(region, startTime, endTime)
	...
	local crf = options.crf
	if
		options.multiple_attempts
		and not options.strict_filesize_constraint
		and format.acceptsBitrate
		and options.target_filesize > 0
		and crf >= 0
	then
		local originalStartTime = startTime
		local originalEndTime = endTime
		...
		while attempt <= 63 do
			...
			if res and crf < 63 then
				...
				if lastSize > 0 then
					...
					delta = math.max(
						math.floor(((size - target) / ((lastSize - size) / ((crf - lastCrf) * expansionFactor))) + 0.5),
						1
					)
				else
					...
					if ratio > 1.1 then
						...
						delta =
							math.max(math.floor((crf * scale) - ((crf * scale) / math.pow(ratio, exponent)) + 0.5), 1)
					end
				end
				lastCrf = crf
...
do
	...
	local _base_0 = {
		...
		draw = function(self)
			...
			ass:new_event()
			self:setup_text(ass)
			ass:append(tostring(bold("t:")) .. " start (" .. tostring(seconds_to_time_string(self.startTime)) .. ")\\N")
			ass:append(tostring(bold("s:")) .. " end (" .. tostring(seconds_to_time_string(self.endTime)) .. ")\\N")
			ass:append(tostring(bold("d:")) .. " goto start time\\N")
			ass:append(tostring(bold("w:")) .. " goto end time\\N")
			ass:append(tostring(bold("o:")) .. " options\\N")
			ass:append(tostring(bold("p:")) .. " preview\\N")
			ass:append(tostring(bold("ENT:")) .. " encode\\N")
			return mp.set_osd_ass(window_w, window_h, ass.text)
		end,
		show = function(self)
